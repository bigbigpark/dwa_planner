...
State::State(double _x, double _y, double _yaw, double _velocity_x, double _velocity_y, double _yawrate)
    : x(_x)
    , y(_y)
    , yaw(_yaw)
...
{
    float cost = fabs(target_velocity - fabs(traj[traj.size() - 1].velocity_x));
    return cost;
}
...
std::vector<State> DWAPlanner::dwa_planning(
...
void DWAPlanner::process(void)
{
    ...
    while (ros::ok()) {
        ...
        if (input_updated && local_goal_subscribed && odom_updated) {
            ...
            ROS_INFO_STREAM("cmd_vel: (" << cmd_vel.linear.x << "[m/s], " << cmd_vel.linear.y << "[m/s], " << cmd_vel.angular.z << "[rad/s])");
